...
export default function ConstructionPage() {
  ...
  const all = allProjects.filter((p) => !["キャンセル", "延期", "請求済み", "交付決定待ち"].includes(p.status));
  ...
  if (loading) return <div style={pageTitle}>🔧 施工管理<br/><span style={{ fontSize: 11, color: "#9ca3af" }}>読み込み中...</span></div>;

  return (
    <div>
      <div style={pageTitle}>🔧 施工管理</div>
      <div style={section}>
        <table style={table}><thead><tr><th style={hcellG}>準備中</th><th style={hcellG}>着工Ready</th><th style={hcellG}>施工中</th><th style={hcellG}>合計</th></tr></thead>
        <tbody><tr>
      ...
      {inProgress.length > 0 && (<div style={section}>
        <div style={{ ...shead, background: "#ecfdf5", color: "#065f46" }}>施工中案件</div>
        <table style={table}><thead><tr><th style={hcell}>案件</th><th style={hcell}>充電器</th><th style={hcell}>施工会社</th><th style={hcell}>着工日</th><th style={hcell}>完工予定</th><th style={hcell}>アラート</th></tr></thead>
        <tbody>{inProgress.map((p) => { const alerts = getDeadlineAlerts(p); return (<tr key={p.id}>
          <td style={cell}><Link href={`/projects/${p.id}`} style={link}>{p.name}</Link><div style={{ fontSize: 9, color: "#9ca3af" }}>{p.caseId}</div></td>
          <td style={cell}>{p.chargerCategory} x{p.quantity}</td><td style={cell}>{p.contractor}</td>
          <td style={cell}>{p.actualStartDate || p.startDate}</td><td style={cell}>{p.endDate}</td>
          <td style={cell}>{alerts.length > 0 ? <span style={{ color: "#dc2626" }}>{alerts[0].message}</span> : "-"}</td>
        </tr>);})}</tbody></table>
      </div>)}
      <div style={section}>
        <div style={shead}>工事前準備 - Ready判定</div>
        <table style={table}><thead><tr><th style={hcell}>案件</th><th style={hcell}>ステータス</th><th style={hcell}>Ready</th><th style={hcell}>不足項目</th><th style={hcell}>資材</th></tr></thead>
        <tbody>{[...ready, ...prep].map((p) => { const rc = checkReadyStatus(p); const mat = getMaterialStatus(p); return (<tr key={p.id}>
          <td style={cell}><Link href={`/projects/${p.id}`} style={link}>{p.name}</Link><div style={{ fontSize: 9, color: "#9ca3af" }}>{p.caseId}</div></td>
      ...
      <div style={{ ...section, background: "#eff6ff", border: "1px solid #bfdbfe" }}>
        <div style={{ ...shead, background: "rgba(191,219,254,0.3)", color: "#1e40af" }}>Kizuku連絡ルール</div>

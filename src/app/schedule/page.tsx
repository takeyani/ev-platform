...
export default function SchedulePage() {
  ...
  return (
    <div>
      ...
      {/* 補助金スケジュール */}
      <div style={section}>
        <div style={shead}>R8年度 NeV補助金 想定スケジュール</div>
        <table style={table}>
          <thead><tr>{["4-6月", "6-9月", "9-10月", "10-12月", "1-2月"].map((p) => <th key={p} style={{ ...hcellG, textAlign: "center" }}>{p}</th>)}</tr></thead>
          <tbody><tr>{["交付申請・決定", "施工期間(1回目)", "実績報告", "施工期間(2回目)", "実績報告(最終)"].map((p) => <td key={p} style={{ ...cell, textAlign: "center", fontSize: 10 }}>{p}</td>)}</tr></tbody>
        </table>
      </div>

      {/* 日程詳細 */}
      <div style={section}>
        <div style={shead}>日程詳細</div>
        <div style={{ overflowX: "auto" }}>
          <table style={table}>
            <thead><tr>
              <th style={hcell}>案件</th><th style={hcell}>状態</th><th style={hcell}>着工前会議</th>
              <th style={hcell}>安全書類</th><th style={hcell}>着工予定</th><th style={hcell}>完工予定</th>
              <th style={hcell}>停電</th><th style={hcell}>受電</th><th style={{ ...hcell, textAlign: "center" }}>Ready</th><th style={{ ...hcell, textAlign: "center" }}>資材</th>
            </tr></thead>
            <tbody>{projects.map((p) => {
              ...
              return (<tr key={p.id}>
                ...
                <td style={{ ...cell, fontWeight: 600 }}>{p.startDate || "-"}</td>

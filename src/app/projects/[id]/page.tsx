...
import { CONSTRUCTION_FLOW_STEPS, SAFETY_DOCUMENTS, SAMPLE_PROJECTS } from "@/lib/constants";
...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    fetchProject(id)
      .then(setP)
      .catch(() => {
        const fallback = SAMPLE_PROJECTS.find((x) => x.id === id);
  ...
  const infoRows = [["充電器種別", p.chargerCategory], ["メーカー/型番", `${p.chargerManufacturer} ${p.chargerModel}`], ["設置台数", `${p.quantity}台`], ["施工会社", p.contractor], ["補助金", p.subsidyType], ["申請区分", p.applicationCategory], ["施工エリア", p.constructionArea], ["案件担当", p.caseManager], ["施工管理", p.constructionManager]];
  const schedRows = [["発注日", p.orderDate], ["着工前会議", p.preConstructionMeetingDate ? `${p.preConstructionMeetingDate} ${p.preConstructionMeetingTime}` : ""], ["安全書類提出", p.safetyDocSubmitDate], ["着工予定日", p.startDate], ["完工予定日", p.endDate], ["電力受電日", p.powerReceptionDate], ["停電日", p.blackoutDate ? `${p.blackoutDate} ${p.blackoutTime}` : ""]];
  const matRows = [["充電器納入希望", p.chargerDeliveryRequestDate], ["充電器納品確認", p.chargerDeliveryConfirmDate], ["看板納品確認", p.signboardDeliveryConfirmDate], ["電材確保確認", p.materialConfirmDate]];
  ...
  return (
    <div>
      ...
      <div style={section}><div style={shead}>工事フロー進捗</div><table style={table}><tbody><tr>

  ...
  return (
    <div>
      ...
      {activeTab === "manage" && !loading && (<div>
        ...
        <div style={section}><div style={shead}>案件別 図面状況</div><table style={table}><thead><tr><th style={hcell}>案件</th><th style={hcell}>充電器</th><th style={hcell}>状態</th>{DRAWING_TYPES.map((d) => <th key={d} style={{ ...hcell, textAlign: "center", fontSize: 10 }}>{d.slice(0, 3)}</th>)}<th style={{...hcell,textAlign:"center"}}>操作</th></tr></thead><tbody>{projects.filter((p)=>!["キャンセル","延期"].includes(p.status)).map((p)=>{const done=p.reportStatus==="承認済み"||p.reportStatus==="提出済み";const wip=["施工中","着工Ready","安全書類提出済み"].includes(p.status);return<tr key={p.id}><td style={cell}><Link href={`/projects/${p.id}`} style={link}>{p.name}</Link><div style={{fontSize:9,color:"#9ca3af"}}>{p.caseId}</div></td><td style={cell}>{p.chargerCategory} x{p.quantity}</td><td style={cell}><span style={statusBadge(p.status)}>{p.status}</span></td>{DRAWING_TYPES.map((dt)=><td key={dt} style={{...cell,textAlign:"center",fontWeight:600,color:done?"#16a34a":wip?"#2563eb":"#d1d5db"}}>{done?"✓":wip?"作成中":"-"}</td>)}<td style={{...cell,textAlign:"center"}}><Link href={`/projects/${p.id}`} style={{fontSize:10,color:"#059669",textDecoration:"none"}}>図面管理</Link></td></tr>})}</tbody></table></div>
      </div>)}

...
import { SAMPLE_PROJECTS, INSPECTION_REQUIREMENTS, DRAWING_TYPES } from "@/lib/constants";
...
export default function ReportsPage() {
  ...
  const reportProjects = SAMPLE_PROJECTS.filter((p) => !["キャンセル", "延期", "交付決定待ち", "交付決定済み"].includes(p.status));
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        {/* 月次サマリー */}
        <div style={section}>
          <div style={shead}>月次検収サマリー（2026年4月）</div>
          <table style={table}>
            <thead><tr><th style={hcellG}>対象</th><th style={hcellG}>未提出</th><th style={hcellG}>承認待</th><th style={hcellG}>差戻</th><th style={hcellG}>請求可</th></tr></thead>
            <tbody><tr>
        ...
        {/* フロー */}
        <div style={section}>
          <div style={shead}>完了報告フロー</div>
          <table style={table}>
            <thead><tr>{["工事完了", "写真撮影", "完成図面", "報告書提出", "テラ検収", "請求送付"].map((s) => <th key={s} style={{ ...hcell, textAlign: "center", fontSize: 10 }}>{s}</th>)}</tr></thead>
            <tbody><tr>{["鍵引渡し", "チェックシート", "4種類", "Kizuku承認依頼", "承認/差戻", "月初2営業日"].map((s) => <td key={s} style={{ ...cell, textAlign: "center", fontSize: 9, color: "#6b7280" }}>{s}</td>)}</tr></tbody>
          </table>
        </div>
      </div>
      ...
      {/* 案件別報告 */}
      <div style={section}>
        <div style={shead}>案件別 完了報告状況</div>
        <table style={table}>
          <thead><tr><th style={hcell}>案件</th><th style={hcell}>状態</th><th style={hcell}>完工日</th><th style={hcell}>報告書提出</th><th style={hcell}>報告書状態</th><th style={hcell}>承認日</th></tr></thead>
          <tbody>{reportProjects.map((p) => (
            <tr key={p.id}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div style={section}>
          <div style={shead}>検収必要書類</div>
          <table style={table}><tbody>{INSPECTION_REQUIREMENTS.documents.map((d) => <tr key={d}><td style={cell}>☐ {d}</td></tr>)}</tbody></table>
        </div>
        <div style={section}>
          <div style={shead}>完成図面（4種類）</div>
          <table style={table}><tbody>{DRAWING_TYPES.map((d) => <tr key={d}><td style={cell}>☐ 完成{d}</td></tr>)}</tbody></table>
        </div>
      </div>
    </div>
  );

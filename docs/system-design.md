...
### Phase 1: 基盤構築 ✅ 完了
- [x] プロジェクト構造（Next.js 16 + TypeScript）
- [x] マスタデータ定義（constants.ts: 14ステータス・14安全書類・7メーカー・60+フィールド）
- [x] 自動化ロジック（automation.ts: Ready判定・アラート・検収集計・資材チェック）
- [x] ダッシュボード（自動集計・アラートパネル・Ready/資材判定列）
- [x] 業務フロー詳細ページ（11ステップ・期限ルール・チェックリスト）
- [x] 案件一覧・詳細・登録（全フィールド対応）
- [x] 施工管理（Ready自動判定・不足項目表示・Kizukuガイド）
- [x] 安全書類管理（14種・案件別提出状況）
- [x] 充電器管理（7メーカー・キュービクル・デバイスID）
- [x] 完了報告（月次検収自動集計・報告フロー）
- [x] スケジュール（ガントチャート・日程詳細）
- [x] 書類管理（安全書類+図面種別+メーカー仕様書）
- [x] 図面管理（Canvas自動生成・エディタ・断面図ビューア）

### Phase 2: DB連携（次期）
- [ ] Supabase DB接続・テーブル作成
- [ ] 認証・権限管理（テラ社員 / 協力会社）
- [ ] 案件CRUD（DB連携）
